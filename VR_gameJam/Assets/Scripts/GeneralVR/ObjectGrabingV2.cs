...
public abstract class ObjectGrabingV2 : HandInteractionBase<ObjectGrabingV2>
{
    ...
    [SerializeField]
    private LayerMask SpringLayers;
    [SerializeField]
    ...
    void OnTriggerStay(Collider col)
    {
        ...
        var inGrabLayer = (layer & GrabableLayers.value) > 0;
        var inSpringLayer = (layer & SpringLayers.value) > 0;
        bool grabbing = GrabTriggerPressedDown();
        if (GrabByCollider && grabbing)
        {
            if (inGrabLayer)
                DoGrabing(col);
            else if (inSpringLayer)
                DoSpringGrabing();
        }
    }

    private void DoSpringGrabing()
    {
